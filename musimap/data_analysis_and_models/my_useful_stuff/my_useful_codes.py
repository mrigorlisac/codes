...
def append_nans_to_timeseries(dataaaray, start_date, end_date, sampling_frequency, mode='beginning'):
    ...
    '''Function to add Nans to the beginning or the end of timeseries because it messes up some statistics later. 
    Output is the dataarray with same dimensions as input, but with added nans.
...
def spatial_pdf_params(dataarray, season='full', threshold=0.1, mode='gamma_pdf'):
    ...
    Season are divided into wet and dry based on calendar months...
...
def average_stations_to_grid_cells(station_data, latbins=None, lonbins=None):
    ...
    Input is: - processed station data with calculated shape parameter of some PDF.
              - bounding box in lat lon to which to translate the station data.
...
def rasterize_shapefiles(path_to_shapefiles, coords_for_burning):
    ...
    DataArray to use it as a blueprint for new DataArray's latitude and longitude into which the catchments will be burned.
...
def spatial_chi_statistic(dataarray, number_of_k_intervals=5, threshold=0.1, mode='genpareto'):
      '''  Function for estimation of Chi square statistic for Maximum likelihood estimator (MLE) per grid cell, 
            using the method with bootstrapping. Input is 3-dim (time, latitude, longitude) xarray dataarray.
            Threshold is to cut the rainfall data. Number of intervals is the number of bins in histogram.  '''
...
def concat_thur_stations_with_metadata(path_to_hydrocode_output_files, mode='my_areas'):
    '''input: path to folder where you have .nc hydrocode out station files, in order to concat them to one file 
...
def P_and_lambda_time_rasters(path_to_dems_for_masks, path_to_rainfall_interpolations_npy, start_year = 1970
    ...
    '''station variable: /Users/lisac/Documents/Data_analysis/Thur/DEMs/nested_catchments_utm/waengi_dem25_utm.txt, path_to_dems_for_masks: base + /*.txt  
...
def calculate_alpha_from_stations(path_to_rainfall_station_data, rain_threshold=0.1):
    '''Function to calculate observed seasonal values of mean rainfall intensity from rainfall station data. Input is
               .txt of rainfall station data, the same format used to interpolate on daily basis. '''
...
def calculate_alpha_from_interpolated_P_rasters(P_timeseries_rasters_datetimeindex_dataframe, rain_threshold=0.1, year_start=1970, year_end=1979):
    '''Calculate mean rainfall intensity from interpolated station data, and summed up in the contributing 6 catchments
        for Thur region.'''
...
def estimate_lambda_from_discharge_timeseries(path_to_discharge_stations_csv_files, parametar='modeled mean rainfall intensity [mm] (E)',
    ...
    '''Seasonal mean discharge and discharge frequency from discharge and climate station data.
...
def create_correlated_daily_rainfall(uncorrelated_rainfall_daily_data, g=1):
    '''First method to create artificial correlated rainfall timeseries on a landscape. It averages the values in a window, 
    i.e. moving square. g is the number of layers around a central square, which value is being 
    ...
    for one_day in range(timeseries_length):
        ...
        for column in range(int(np.ceil(ny/n))):
            ...
            for row in range(int(np.ceil(nx/n))):
                test2[one_day, starty:endy, startx:endx] = stats.expon.rvs(loc=0, scale=scale_param, size=1)
                startx += n
                endx += n
